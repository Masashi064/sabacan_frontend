-- ============================================================
-- Migration: more specific recommendation reason labels
-- Apply via: Supabase Dashboard > SQL Editor > Run
-- ============================================================

-- ============================================================
-- get_recommended_articles(): re-declared (same signature) to make
-- the "reason" label name the actual matched signal instead of a
-- generic "Recommended for You" — e.g. "⭐ Because you like Business"
-- when a favorite-category match drove the score. Deliberately does
-- NOT add labels like "Trending" or "Continue Learning" since there
-- is no popularity/partial-progress data backing them yet — every
-- label here still reflects a real signal already computed above.
-- ============================================================
...
BEGIN
  ...
  SELECT
    ...
    s.video_length,
    -- Priority order for the label: explicit category match names the
    -- category (most specific/explainable), then channel affinity
    -- (explicit or inferred), then recency, then a neutral fallback.
    CASE
      WHEN s.category_score > 0 THEN '⭐ Because you like ' || s.assigned_category
      WHEN s.channel_score > 0 OR s.history_channel_score > 0 THEN '🎯 Recommended for You'
